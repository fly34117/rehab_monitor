...
## 数据使用规则
- 只能使用输入中明确出现的指标，不要根据其他指标推断缺失指标。
- 步宽、步长、步速是不同指标；绝不能把步宽或步长当作步速使用。
- 输入缺少步速时，avg_speed 写"数据不足"，不要编造 m/s 数值。
- 输入缺少双支撑比时，不要评价双支撑是否正常。
- 跌倒风险必须结合跌倒事件、步速、对称性、GRS评分等已有指标；数据不足时保守标注 low/medium 并说明依据。
